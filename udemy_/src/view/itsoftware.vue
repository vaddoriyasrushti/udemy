<template>
  ...
  <template v-if="$route.name!='home'">
    ...
    <h2 class="header_name">{{title}}</h2>
      ...
      <div class="cols">
        <v-flex v-if="getitsoftware.length==0">
...
export default {
  ...
  computed: {
    ...
    getsubtosubcategories() {
      var subtosubcat = [];
      this.$store.state.categories.itsoftware.map(subcat => {
    ...
    filteredList() {
      ...
      return this.$store.state.categories.itsoftware.filter(function(customer) {
