...
<div class="cols">
    <v-flex v-if="getpersonaldevelopment.length==0">
        ...
        <template slot="content">
         <div>
            <center><a-button
              ...
              @click="$router.push({name:'cart'})">Go to cart</a-button>
            <a-button type="primary" v-else @click="$store.dispatch('addCourse',i);">Add to cart</a-button></center>
          </div>
        </template>
      <v-card @click="$router.push({name:'detail',params:{course:i.catname+'_'+i.populartopic},query:{id:i.id}})"> 
        <v-img
        :src=" 'http://localhost:3003/images/'+ i.topicimage " alt="gvb" height="150px"
        >
        </v-img>
        ...
        <v-card-actions>
          <b><div style="font-size:17px">₹{{i.price}}</div></b>
          <v-spacer></v-spacer>
       </v-card-actions>
        </v-card>
      </a-popover>
...
export default {
    ...
    computed:{
      ...
      var subtosubcat = [];
      this.$store.state.categories.personaldevelopment.map(subcat => {
    ...
    filteredList() {
      ...
      return this.$store.state.categories.personaldevelopment.filter(function(
        customer
      ) {
