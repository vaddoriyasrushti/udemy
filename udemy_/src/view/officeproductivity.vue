...
<div class="cols">
    <v-flex v-if="getofficeproductivity.length==0">
...
export default {
  ...
  computed: {
    ...
    getsubtosubcategories() {
      var subtosubcat = [];
      this.$store.state.categories.officeproductivity.map(subcat => {
    ...
    filteredList() {
      ...
      return this.$store.state.categories.officeproductivity.filter(function(
        customer
      ) {
