...
<div class="cols">
    ...
    <div class="card h-400">
      <a-popover :title="i.description" placement="right">
        ...
        <!-- @click="$router.push({name:i.populartopic})" -->
  ...
  computed: {
    getmusic() {
     ...
     getsubtosubcategories(){
      var subtosubcat=[];
      this.$store.state.categories.music.map((subcat)=>{
    ...
    filteredList() {
      ...
      return this.$store.state.categories.music.filter(function(
        customer
      ) {
        return (
